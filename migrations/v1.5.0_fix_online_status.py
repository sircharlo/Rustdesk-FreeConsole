...
def backup_database(db_path):
    ...
    try:
        ...
        if auto_mode:
            ...
            print(
                "Press 'y' and Enter to continue without backup, or any other key to cancel."
            )
...
def add_or_fix_column(cursor, table, column, column_type, default=None):
    ...
    if column in columns:
        ...
        if current_type == column_type.upper() or (
            current_type == "" and column_type == "TEXT"
        ):
            # SQLite sometimes returns empty type for some columns if created weirdly,
            # but usually it's explicit. We'll stick to strict or loose check.
            print(
    ...
    try:
        cursor.execute(sql)
        print(f"  ✓ Added/Fixed column '{column}'")
        return True
    except Exception as e:
...
def run_migration(db_path):
    ...
    # Add status_online column (critical for online status detection)
    if add_or_fix_column(cursor, "peer", "status_online", "INTEGER", 0):
        changes_made += 1

    # Add is_deleted column (for soft delete)
    if add_or_fix_column(cursor, "peer", "is_deleted", "INTEGER", 0):
        changes_made += 1

    # Add ban-related columns
    if add_or_fix_column(cursor, "peer", "is_banned", "INTEGER", 0):
        changes_made += 1

    if add_or_fix_column(cursor, "peer", "banned_at", "TEXT"):
        changes_made += 1

    if add_or_fix_column(cursor, "peer", "banned_by", "TEXT"):
        changes_made += 1

    if add_or_fix_column(cursor, "peer", "ban_reason", "TEXT"):
        changes_made += 1
