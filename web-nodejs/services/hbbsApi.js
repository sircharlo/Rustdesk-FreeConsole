...
async function changePeerId(oldId, newId) {
  try {
    const { data } = await apiClient.post(`/peers/${oldId}/change-id`, {
      new_id: newId,
    });
    return data;
  } catch (err) {
...
async function syncOnlineStatus(db) {
  try {
    ...
    console.log("HBBS API online peers:", onlinePeers);
    ...
    if (onlinePeers.length === 0) {
      ...
      if (health.status !== "running") {
        console.warn(
          "HBBS API unreachable - skipping status sync to preserve current state",
        );
    ...
    if (onlineIds.size > 0) {
      const placeholders = new Array(onlineIds.size).fill("?").join(",");
      db.prepare(
        `UPDATE peer SET status_online = 1, last_online = datetime('now') WHERE id IN (${placeholders})`,
      ).run(...onlineIds);
    }
