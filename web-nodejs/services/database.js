...
function getDb() {
  if (!db) {
    try {
      console.log(`DB: Connecting to RustDesk database: ${config.dbPath}`);
      ...
      console.error(
        `DB Error: Failed to connect to RustDesk database: ${err.message}`,
      );
      throw err;
    }
  }
...
function getAuthDb() {
  if (!authDb) {
    try {
      ...
      console.log(`DB: Connecting to Auth database: ${authDbPath}`);
      ...
      console.error(
        `DB Error: Failed to connect to Auth database: ${err.message}`,
      );
      throw err;
    }
  }
...
function ensurePeerTable(db) {
  ...
  const columns = [
    ...
    { name: "status", sql: "INTEGER DEFAULT NULL" },
    { name: "last_online", sql: "DATETIME DEFAULT NULL" },
    { name: "is_deleted", sql: "INTEGER DEFAULT 0" },
    { name: "user", sql: "TEXT DEFAULT ''" },
    { name: "is_banned", sql: "INTEGER DEFAULT 0" },
    { name: "banned_at", sql: "DATETIME DEFAULT NULL" },
...
function parseDeviceInfo(device) {
  ...
  return {
    ...
    online:
      device.status === 1 ||
      (device.status === null && device.status_online === 1),
...
function getAllDevices(filters = {}) {
  ...
  if (filters.status === "online") {
    sql += " AND (status = 1 OR (status IS NULL AND status_online = 1))";
  } else if (filters.status === "offline") {
    sql +=
      " AND (status = 0 OR (status IS NULL AND status_online = 0)) AND is_banned = 0";
...
function getDeviceById(id) {
  const device = getDb()
    .prepare("SELECT * FROM peer WHERE id = ? AND is_deleted = 0")
    .get(id);
...
function updateDevice(id, data) {
  ...
  params.push(id);
  return getDb()
    .prepare(`UPDATE peer SET ${fields.join(", ")} WHERE id = ?`)
    .run(...params);
}
...
function getStats() {
  const db = getDb();
  const total = db
    .prepare("SELECT COUNT(*) as count FROM peer WHERE is_deleted = 0")
    .get().count;
  const online = db
    .prepare(
      "SELECT COUNT(*) as count FROM peer WHERE is_deleted = 0 AND (status = 1 OR (status IS NULL AND status_online = 1))",
    )
    .get().count;
  const banned = db
    .prepare(
      "SELECT COUNT(*) as count FROM peer WHERE is_deleted = 0 AND is_banned = 1",
    )
    .get().count;
  const withNotes = db
    .prepare(
      "SELECT COUNT(*) as count FROM peer WHERE is_deleted = 0 AND note IS NOT NULL AND note != ''",
    )
    .get().count;
...
function countDevices(filters = {}) {
  ...
  if (filters.status === "online") {
    ...
    sql += " AND (status = 0 OR (status IS NULL AND status_online = 0))";
  } else if (filters.status === "banned") {
    sql += " AND is_banned = 1";
  }
...
function hasUsers() {
  return (
    getAuthDb().prepare("SELECT COUNT(*) as count FROM users").get().count > 0
  );
}
...
function createFolder(name, color, icon) {
  return getAuthDb()
    .prepare("INSERT INTO folders (name, color, icon) VALUES (?, ?, ?)")
    .run(name, color, icon);
}
...
function assignDeviceToFolder(deviceId, folderId) {
  return getDb()
    .prepare("UPDATE peer SET folder_id = ? WHERE id = ?")
    .run(folderId, deviceId);
}
...
function unassignDevicesFromFolder(folderId) {
  return getDb()
    .prepare("UPDATE peer SET folder_id = NULL WHERE folder_id = ?")
    .run(folderId);
}
...
function revokeAllUserTokens(userId) {
  return getAuthDb()
    .prepare("UPDATE access_tokens SET revoked = 1 WHERE user_id = ?")
    .run(userId);
}
...
function clearAccountLockout(username) {
  return getAuthDb()
    .prepare("DELETE FROM account_lockouts WHERE username = ?")
    .run(username);
}
...
function getAddressBook(userId, abType = "legacy") {
  const row = getAuthDb()
    .prepare("SELECT data FROM address_books WHERE user_id = ? AND ab_type = ?")
    .get(userId, abType);
  return row ? row.data : "{}";
}
