...
if (fs.existsSync(_envFile)) {
  try {
    ...
    /* .env read failed — continue with existing env vars */
  }
}
...
const DATA_DIR =
  ...
  (isDocker ? "/app/data" : path.join(__dirname, "..", "data"));
const KEYS_PATH =
  process.env.KEYS_PATH ||
  process.env.RUSTDESK_DIR ||
  process.env.RUSTDESK_PATH ||
  (isDocker ? "/opt/rustdesk" : isWindows ? "C:\\BetterDesk" : "/opt/rustdesk");
...
// Key paths
const PUB_KEY_PATH =
  process.env.PUB_KEY_PATH || path.join(KEYS_PATH, "id_ed25519.pub");
const API_KEY_PATH =
  process.env.API_KEY_PATH || path.join(KEYS_PATH, ".api_key");
...
module.exports = {
  ...
  httpRedirect:
    (process.env.HTTP_REDIRECT_HTTPS || "true").toLowerCase() === "true",
  ...
  // HBBS API
  hbbsApiUrl: process.env.HBBS_API_URL || "http://localhost:21120/api",
  ...
  sessionMaxAge:
    parseInt(process.env.SESSION_MAX_AGE, 10) || 24 * 60 * 60 * 1000, // 24 hours

  // Rate limiting
  rateLimitWindowMs:
    parseInt(process.env.RATE_LIMIT_WINDOW_MS, 10) || 60 * 1000, // 1 minute
