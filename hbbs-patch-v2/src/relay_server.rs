    ...
    if let Ok(Some(Ok(bytes))) = timeout(30_000, stream.recv()).await {
        if let Ok(msg_in) = RendezvousMessage::parse_from_bytes(&bytes) {
            if let Some(rendezvous_message::Union::RequestRelay(rf)) = msg_in.union {
                ...
                if !rf.uuid.is_empty() {
                    // BAN CHECK: Block relay ONLY if the TARGET device is banned
                    // We only check target_id (rf.id), NOT all devices from the same IP
                    // This prevents blocking all devices just because one device on the same IP is banned
                    // Note: RequestRelay doesn't contain initiating peer's ID, only target ID
                    ...
                    let is_banned = hbb_common::tokio::task::spawn_blocking(move || {
                        ...
                        match Connection::open_with_flags(
                            ...
                            Ok(conn) => {
                                ...
                                // This protects banned devices from receiving connections
                                ...
                                // Device is not banned - allow relay
                                false
                            }
                            Err(e) => {
                                log::debug!("HBBR ban check DB error (allowing relay): {}", e);
                                false // If DB fails, allow relay (fail-open)
                            }
                        }
                    }).await;
